  ...
  class Url
    ...
    def display
      ...
      unless stack_exists?(stack_name)
        puts "Stack for #{Jets.config.project_name.colour(:green)} project for environment #{Jets.env.colour(:green)}.  Couldn't find #{stack_name.colour(:green)} stack."
        exit
      end
      ...
      if api_gateway_stack_arn && endpoint_available?
        ...
        puts "API Gateway not found. This jets app does have an API Gateway associated with it.  Please double check your config/routes.rb if you were expecting to see a url for the app. Also check that #{stack_name.colour(:green)} is a jets app."
      end
    end
